...
if __name__ == '__main__':
    ...
    hidden_size = 8
    w = Init.xavier((hidden_size + 1, hidden_size), is_grad=True)

    wz = Init.xavier((hidden_size + 1, hidden_size), is_grad=True)

    wr = Init.xavier((hidden_size + 1, hidden_size), is_grad=True)
    ...
    for i in range(epoch):
        ...
        oh = Layers.gru(x, h, w, wz, wr)
        yp = oh @ v + b

        loss = Losses.mse(yp, y)

        for weight in [w, wz, wr, v, b]:
            weight.grad_zeros()

        backward(loss)
        adam.run([w, wz, wr, v, b])
